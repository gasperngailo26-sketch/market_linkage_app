...
RegisteredUser? findRegisteredUser(String email) {}
...
class _SecurityQuestionDialogState extends State<SecurityQuestionDialog> {
  ...
  VoidCallback? get resetPassword => null; // 0: Question, 1: Set new password
  ...
  Future<void> _resetPassword(dynamic registeredUsers) async {
    ...
    // 1. Find where this user is in your list
    final index = registeredUsers.indexWhere(
      (u) => u.username == widget.user.username,
    );

    if (index != -1) {
      // 2. Update the password in that list
      registeredUsers[index] = registeredUsers[index].copyWith(
        password: newPassword,
      );

      // 3. Save it to the phone so they can login with the new password later
      await saveRegisteredUsers();
